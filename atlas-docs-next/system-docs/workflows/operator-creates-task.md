...
4. Client builds a `POST /tasks` request with `asset_id`, `json.components.command.type`, and `json.components.parameters`.
5. Atlas Core resolves and pins the active catalog object, then validates the target asset, command type, command parameters, and required supported command capability.
6. Atlas Core creates the task with `status: "pending"` and server-assigned `command_catalog_object_id`.
7. Atlas Core publishes `task.created`.

## Failure Handling

- Invalid command type or parameters return `400 command_validation_failed`.
- A caller-supplied `command_catalog_object_id` returns `400 validation_failed`.
