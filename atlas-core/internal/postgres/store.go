...
func (s *Store) CountEntityDependents(ctx context.Context, id string) (map[string]int, error) {
    ...
    if count, err := s.count(ctx, `SELECT count(*) FROM tasks WHERE asset_id=$1`, id); err != nil { return nil, err } else { counts["tasks"] = count }
    if count, err := s.count(ctx, `SELECT count(*) FROM observations WHERE source_asset_id=$1`, id); err != nil { return nil, err } else { counts["observations"] = count }
    if count, err := s.count(ctx, `SELECT count(*) FROM objects WHERE owner_type='entity' AND owner_id=$1`, id); err != nil { return nil, err } else { counts["objects"] = count }
    return counts, nil
}
...
func (s *Store) UpdateObservation(ctx context.Context, observation model.Observation) (model.Observation, error) {
    ...
    if err != nil { return model.Observation{}, err }
    if result.RowsAffected() == 0 { return model.Observation{}, model.NotFound("observation", observation.ObservationID) }
...
func (s *Store) DeleteObservation(ctx context.Context, id string) error {
    ...
    if err != nil { return err }
    if result.RowsAffected() == 0 { return model.NotFound("observation", id) }
    if _, err := tx.Exec(ctx, `DELETE FROM objects WHERE owner_type='observation' AND owner_id=$1`, id); err != nil { return err }
    if err := tx.Commit(ctx); err != nil { return err }
    for _, path := range paths { _ = s.files.Delete(path) }
    return nil
}
...
func (s *Store) GetTask(ctx context.Context, id string) (model.Task, error) {
    ...
    if err != nil { return model.Task{}, mapNotFound(err, "task", id) }
    return task, nil
}
...
func (s *Store) ListTasks(ctx context.Context, filter store.TaskListFilter, pagination model.Pagination) ([]model.Task, int, error) {
    clauses := []string{}
    args := []any{}
    if filter.AssetID != "" { args = append(args, filter.AssetID); clauses = append(clauses, fmt.Sprintf("asset_id=$%d", len(args))) }
    if filter.Status != "" { args = append(args, filter.Status); clauses = append(clauses, fmt.Sprintf("status=$%d", len(args))) }
    where := joinWhere(clauses)
...
func (s *Store) UpdateTask(ctx context.Context, task model.Task) (model.Task, error) {
    ...
    if err != nil { return model.Task{}, err }
    if result.RowsAffected() == 0 { return model.Task{}, model.NotFound("task", task.TaskID) }
...
func (s *Store) DeleteTask(ctx context.Context, id string) error {
    ...
    if err != nil { return err }
    if result.RowsAffected() == 0 { return model.NotFound("task", id) }
    return nil
}
...
func (s *Store) GetObject(ctx context.Context, id string) (model.Object, error) {
    ...
    if err != nil { return model.Object{}, mapNotFound(err, "object", id) }
    return object, nil
}
...
func (s *Store) ListObjects(ctx context.Context, filter store.ObjectListFilter, pagination model.Pagination) ([]model.Object, int, error) {
    ...
    if filter.Type != "" { args = append(args, filter.Type); clauses = append(clauses, fmt.Sprintf("type=$%d", len(args))) }
    where := joinWhere(clauses)
...
func (s *Store) UpdateObject(ctx context.Context, object model.Object) (model.Object, error) {
    ...
    if err != nil { return model.Object{}, err }
    if result.RowsAffected() == 0 { return model.Object{}, model.NotFound("object", object.ObjectID) }
...
func (s *Store) DeleteObject(ctx context.Context, id string) error {
    ...
    if err != nil { return err }
    if result.RowsAffected() == 0 { return model.NotFound("object", id) }
    if err := tx.Commit(ctx); err != nil { return err }
    for _, path := range paths { _ = s.files.Delete(path) }
    return nil
}
...
func (s *Store) CreateObjectFile(ctx context.Context, input store.ObjectUploadInput) (model.ObjectFile, error) {
    ...
    if err != nil { return model.ObjectFile{}, err }
    defer os.Remove(stagedPath)
    file := input.File
    file.Path = s.files.LogicalPath(file.ObjectID, file.FileID)
    file.SizeBytes = size
    if file.ContentType == "" { file.ContentType = detectedType }
    now := time.Now().UTC()
    file.CreatedAt = now
    file.UpdatedAt = now
    tx, err := s.pool.BeginTx(ctx, pgx.TxOptions{})
    if err != nil { return model.ObjectFile{}, err }
    defer tx.Rollback(ctx)
    if err := ensureObjectExists(ctx, tx, file.ObjectID); err != nil { return model.ObjectFile{}, err }
    ...
    if _, err := tx.Exec(ctx, `UPDATE objects SET updated_at=$2 WHERE object_id=$1`, file.ObjectID, now); err != nil { return model.ObjectFile{}, err }
    if err := tx.Commit(ctx); err != nil { return model.ObjectFile{}, err }
...
func (s *Store) GetObjectFile(ctx context.Context, objectID, fileID string) (model.ObjectFile, error) {
    ...
    if err != nil { return model.ObjectFile{}, mapNotFound(err, "object_file", fileID) }
    return file, nil
}
...
func (s *Store) AppendObjectFile(ctx context.Context, objectID, fileID string, reader io.Reader, maxBytes int64) (model.ObjectFile, error) {
    ...
    if err != nil { return model.ObjectFile{}, err }
    defer tx.Rollback(ctx)
    if _, err := tx.Exec(ctx, `UPDATE object_files SET size_bytes=$3, updated_at=$4 WHERE file_id=$1 AND object_id=$2`, fileID, objectID, newSize, now); err != nil { return model.ObjectFile{}, err }
    if _, err := tx.Exec(ctx, `UPDATE objects SET updated_at=$2 WHERE object_id=$1`, objectID, now); err != nil { return model.ObjectFile{}, err }
...
func (s *Store) OpenObjectFile(ctx context.Context, objectID, fileID string) (model.ObjectFile, io.ReadCloser, error) {
    file, err := s.GetObjectFile(ctx, objectID, fileID)
    if err != nil { return model.ObjectFile{}, nil, err }
...
func (s *Store) DeleteObjectFile(ctx context.Context, objectID, fileID string) error {
    ...
    if err != nil { return err }
    if result.RowsAffected() == 0 { return model.NotFound("object_file", fileID) }
    if _, err := tx.Exec(ctx, `UPDATE objects SET updated_at=$2 WHERE object_id=$1`, objectID, time.Now().UTC()); err != nil { return err }
    if err := tx.Commit(ctx); err != nil { return err }
...
func (s *Store) GetFullQueryState(ctx context.Context) (store.QueryState, error) {
    ...
    if err != nil { return store.QueryState{}, err }
    defer tx.Rollback(ctx)
    state := store.QueryState{}
    if rows, err := tx.Query(ctx, `SELECT entity_id, type, COALESCE(subtype,''), COALESCE(alias,''), json, created_at, updated_at FROM entities ORDER BY updated_at DESC, entity_id ASC`); err != nil { return state, err } else { state.Entities, _, err = collectEntities(rows); if err != nil { return state, err } }
    if rows, err := tx.Query(ctx, `SELECT observation_id, source_asset_id, json, created_at, updated_at FROM observations ORDER BY updated_at DESC, observation_id ASC`); err != nil { return state, err } else { state.Observations, _, err = collectObservations(rows, 0); if err != nil { return state, err } }
    if rows, err := tx.Query(ctx, `SELECT task_id, status, asset_id, command_catalog_object_id, json, created_at, updated_at FROM tasks ORDER BY updated_at DESC, task_id ASC`); err != nil { return state, err } else { state.Tasks, _, err = collectTasks(rows, 0); if err != nil { return state, err } }
    if rows, err := tx.Query(ctx, `SELECT object_id, type, owner_type, owner_id, json, created_at, updated_at FROM objects ORDER BY updated_at DESC, object_id ASC`); err != nil { return state, err } else { state.Objects, _, err = collectObjects(rows, 0); if err != nil { return state, err } }
    if rows, err := tx.Query(ctx, `SELECT file_id, object_id, path, content_type, size_bytes, COALESCE(usage_hint,''), created_at, updated_at FROM object_files ORDER BY updated_at DESC, file_id ASC`); err != nil { return state, err } else { state.ObjectFiles, err = collectObjectFiles(rows); if err != nil { return state, err } }
    if err := tx.Commit(ctx); err != nil { return state, err }
    return state, nil
}
...
func scanEntity(scanner interface{ Scan(dest ...any) error }) (model.Entity, error) {
    var raw []byte
    entity := model.Entity{}
    if err := scanner.Scan(&entity.EntityID, &entity.Type, &entity.Subtype, &entity.Alias, &raw, &entity.CreatedAt, &entity.UpdatedAt); err != nil { return model.Entity{}, err }
...
func scanObservation(scanner interface{ Scan(dest ...any) error }) (model.Observation, error) {
    ...
    if err := scanner.Scan(&item.ObservationID, &item.SourceAssetID, &raw, &item.CreatedAt, &item.UpdatedAt); err != nil { return model.Observation{}, err }
...
func scanTask(scanner interface{ Scan(dest ...any) error }) (model.Task, error) {
    var raw []byte
    item := model.Task{}
    if err := scanner.Scan(&item.TaskID, &item.Status, &item.AssetID, &item.CommandCatalogObjectID, &raw, &item.CreatedAt, &item.UpdatedAt); err != nil { return model.Task{}, err }
    item.JSON = unmarshalJSON(raw)
    return item, nil
}
...
func scanObject(scanner interface{ Scan(dest ...any) error }) (model.Object, error) {
    var raw []byte
    item := model.Object{}
    if err := scanner.Scan(&item.ObjectID, &item.Type, &item.OwnerType, &item.OwnerID, &raw, &item.CreatedAt, &item.UpdatedAt); err != nil { return model.Object{}, err }
    item.JSON = unmarshalJSON(raw)
    return item, nil
}
...
func scanObjectFile(scanner interface{ Scan(dest ...any) error }) (model.ObjectFile, error) {
    ...
    if err := scanner.Scan(&item.FileID, &item.ObjectID, &item.Path, &item.ContentType, &item.SizeBytes, &item.UsageHint, &item.CreatedAt, &item.UpdatedAt); err != nil { return model.ObjectFile{}, err }
    return item, nil
}
...
func unmarshalJSON(raw []byte) model.JSONMap {
    if len(raw) == 0 { return model.JSONMap{} }
...
func chooseLimit(requested, fallback int64) int64 {
    if requested > 0 && requested < fallback { return requested }
    return fallback
}
