...
- Running `go test ./...` from `atlas-core/` can leave an untracked `atlas-core/atlas-core` binary in the workspace; delete it before committing so progress-report commits do not accidentally capture the built executable.
